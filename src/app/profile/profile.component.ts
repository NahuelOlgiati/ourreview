...
export class ProfileComponent implements OnInit {
  ...
  constructor(private userService: UserService, private snackbar: MdSnackBar, private router: Router, private dialog: MdDialog) { }
  ...
  deleteAccount() {
    ...
    this.userService.deleteUser((error) => {
      if (error) {
        this.error = error;
  ...
  onSubmit(formData) {
    if (formData.valid) {
      this.userService.updateUser(formData, (error) => {
        if (error) {
          this.error = error;
          this.snackbar.open(this.error, 'hide', { duration: 10000 });
        } else {
  ...
  ngOnInit() {
    this.userService.readUser().subscribe(authData => {
      if (authData) {
